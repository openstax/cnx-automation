...
def test_corgi_pdf_job_e2e(chrome_browser, chrome_page):
    ...
    new_job.click_pdf_radio_button()

    new_job.fill_collection_id("col11992")
    new_job.fill_version("latest")
    new_job.fill_book_style("astronomy")
    new_job.click_content_server()
    new_job.click_content_server_dropdown("production")

    new_job.click_create_button_pdf()

    new_job.start_date_time.wait_for()
    new_job.job_state_completed.wait_for()
    new_job.pdf_view_link.wait_for()

    job_id = new_job.job_id_locator.text_content()

    with chrome_page.expect_popup() as popup_info:
        new_job.pdf_view_link.click()
