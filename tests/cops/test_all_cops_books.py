...
@pytest.mark.xfail(raises=(AssertionError, HTTPError), strict=False)
@markers.awss3
@markers.nondestructive
def test_verify_collections_in_s3_bucket(
    selenium, s3_books_full_url_list, s3_books_title_version_dict, s3_books_uuids_list
):
    ...
    for books in range(len(s3_books_full_url_list)):
        ...
        try:
            ...
            try:
                ...
                s3_uuid = s3_jdata.get("id")
                slug_pref = s3_nest[0]["slug"]
                last_chap = s3_nest[-1]
                slug_index = last_chap["slug"]
                s3_version = "1." + s3_jdata.get("version")

                random_entry = random.choice(s3_nest)
                random_slug = random_entry["slug"]
                ...
                if slug_pref == "preface" and slug_index == "index":
                    ...
                    assert random_slug != ""
                    assert s3_uuid in s3_books_uuids_list

                    assert 200 == response.status_code

                else:
                    assert "preface" in slug_pref
                    assert s3_title in s3_books_title_version_dict.keys()
                    assert s3_version in s3_books_title_version_dict.values()
                    assert random_slug != ""
                    assert s3_uuid in s3_books_uuids_list
