...
cnx_lean_dump.sql.gz:
	scp backup2.cnx.org:`ssh backup2.cnx.org 'ls -t /var/backups/cnx_lean_dump.*.sql.gz | awk "{print $1; exit}"'` cnx_lean_dump.sql.gz
...
test-webview: cnx_lean_dump.sql.gz $(STATEDIR)/docker-build
