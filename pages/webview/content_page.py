...
class ContentPage(Content):
    ...
    def loaded(self):
        return (
            bool(self._url_regex.search(self.driver.current_url))
            and self.books_containing.overview_is_present
        )
    ...
    class BooksContaining(Region):
        ...
        def date_list(self):
            return [
                self.Book(self.page, el).revision_date.text.split(":")[1:]
        ...
        def author_list(self):
            return [
                (self.Book(self.page, el).author.text.split(": ")[1:])
                for el in self.find_elements(*self._book_list_locator)
            ]
        ...
        class Book(Region):
            ...
            _go_to_book_locator = (By.CSS_SELECTOR, "ul > li > ul > li:nth-child(3) > div > a")
